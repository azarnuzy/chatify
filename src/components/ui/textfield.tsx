...
export const TextField = <T extends FieldValues>({
  ...
  const inputStatus = clsx({
    'focus:ring-1 focus:ring-error-base  ring-1 ring-error-base text-sm': status === 'error',
  ...
  return (
    <section className="flex flex-col w-auto my-1 gap-y-2 ">
      ...
      <section className="relative flex items-center w-auto">
        {props.prepend && (
          <label className="items-center inset-0 absolute flex items justify-center w-[40px]" htmlFor={props.name}>
            {props.prepend}
          </label>
        )}
        {!isTextArea ? (
